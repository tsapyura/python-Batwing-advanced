...
def create(book_id, author_id):
    if Book.query.filter(Book.id == book_id).first() and Author.query.filter(Author.id == author_id).first():
        new_relation = AuthorOfBook(name_of_book=book_id, author=author_id)
        db.session.add(new_relation)
        db.session.commit()
        new_relation_json = AuthorsOfBooksSchema().dump(new_relation)
